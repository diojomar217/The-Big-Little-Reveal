...
import type { Gender, PublicState, VoteChoice } from "@/lib/reveal-types";

type EventStatus = PublicState["status"];
type Vote = { choice: VoteChoice; name: string };
type EventRecord = { code: string; title: string; ownerEmail: string; status: EventStatus; gender: Gender | null; revealAt: string | null; votes: Map<string, Vote> };
...
async function ensureSchema() {
  ...
  schemaPromise ??= (async () => {
    await sql`CREATE TABLE IF NOT EXISTS reveal_events (code TEXT PRIMARY KEY, title TEXT NOT NULL, owner_email TEXT, passcode_hash TEXT, status TEXT NOT NULL DEFAULT 'standby', gender TEXT, reveal_at TIMESTAMPTZ, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`;
...
export async function createEvent(title: string, ownerEmail: string) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    const code = makeCode();
    if (!sql) { if (events.has(code)) continue; events.set(code, { code, title: cleanTitle, ownerEmail: owner, status: "standby", gender: null, revealAt: null, votes: new Map() }); return { code, title: cleanTitle }; }
...
export async function getPublicState(code: string, voterId?: string): Promise<PublicState | null> {
  ...
  if (!sql) {
    ...
    return { title: event.title, status, gender: status === "revealed" ? event.gender : null, revealAt: event.revealAt, votes: { girl, boy }, total: girl + boy, yourVote: voterId ? event.votes.get(voterId)?.choice ?? null : null };
  }
  ...
  const eventRows = await sql`SELECT title, status, gender, reveal_at FROM reveal_events WHERE code = ${normalized}`; if (!eventRows.length) return null;
  ...
  return { title: String(event.title), status, gender: status === "revealed" ? event.gender as Gender : null, revealAt: event.reveal_at ? new Date(event.reveal_at as string).toISOString() : null, votes, total: votes.girl + votes.boy, yourVote: (voteRows[0]?.choice as VoteChoice | undefined) ?? null };
}
...
export async function startReveal(code: string) {
  const normalized = normalizeCode(code), revealAt = new Date(Date.now() + 9000).toISOString(), sql = database();
